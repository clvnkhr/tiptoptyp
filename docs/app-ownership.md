...
| Find flags/query/replacement, `SearchSession` | `find_bar.rs` owns the window-local presentation state and revision-keyed cache; `editor_view.rs` supplies borrowed source/key and the root transition/shortcut adapters clear it. | The bar returns navigation/replacement intents; document edits use `DocumentSession`, and navigation distinguishes Search from Focus. |
...
| Shortcut query/capture/notice, overrides visibility, pending paste | `shortcut_editor.rs` owns the child presentation state and returns typed actions; root shortcut capture, `settings_view` and `extra_shortcuts` retain input/persistence ownership. | Capture consumes input before commands; the editor paints a borrowed settings snapshot and widget edits must not mutate the document behind them. |
...
| 249 | `show_shortcut_editor_window` passed query/capture/notice into an unrestricted helper; root capture and extra-shortcut open/close wrote the same presentation state. | Complete: `shortcut_editor.rs` renders borrowed settings and returns typed actions; persistence and singleton broadcasts remain in `settings_view`/root. Isolation and boundary tests cover the child without an app owner. |
| 250 | `show_find_bar`, `apply_find_actions`, `open_find`/`toggle_find`, Escape and extra shortcuts repeated close/case/regex/cache transitions. | Complete: `find_bar.rs` owns query/control state and the existing `SearchSession`, returning control intents while document mutation and `EditorSelection` stay in the window owner. Existing focus, Unicode, stale-identity and cache tests remain. Native acceptance still needs 237. |
