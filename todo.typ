...
161. [ ] after we have collapsed some lines, editing in the code panel causes us to jump to the wrong line. Similarly with the jump-to-source from the preview panel.
162. [ ] possibly related to above: with collapsed lines, when we type, we see the collapsed lines reappear for a split second.
163. [ ] Allow for the pdf preview to pop out into a different window. if I close it, the preview should go back to the window
164. [ ] Somethings wrong with window shadows - when a ttt window is focused, there is no shadow outside the window boundaries. Oddly, this is not the case for the settings window and any of its child windows.
