...
157. [ ]  implement collapsing multiline code with a compact dropdown arrow at the line number. the dropdown should not increase the width of the line number margin-put it in the tiny space between line number and the git hunk marker. make it toggle on line number click as well. we need to collapse all the same things as the sticky rows feature
158. [ ] collapsing multiline and sticky rows need to work for md and tex codeblocks/strings as well (cmarker, mitex). If you need to add a parser/lsp for tex and md, go for it: find a good rust one and do it.
