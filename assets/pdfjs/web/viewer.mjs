...
class PDFLinkService {
  ...
  async goToDestination(dest) {
    ...
    // tiptoptyp: a pending destination/focus belongs to this PDF generation.
    const destinationDocument = this.pdfDocument;
    ...
    if (this.pdfDocument !== destinationDocument) {
      return;
    }
    if (this.pdfHistory) {
    ...
    this.eventBus.on("textlayerrendered", evt => {
      ...
      if (evt.pageNumber === pageNumber) {
        // The user may have scrolled since clicking the link. Late text
        // rendering must not scroll them back (or steal the editor's focus).
        if (this.pdfViewer.currentPageNumber === pageNumber && this.pdfViewer.containsElement(document.activeElement)) {
          evt.source.textLayer.div.focus({ preventScroll: true });
        }
        ac.abort();
      }
    }, {
