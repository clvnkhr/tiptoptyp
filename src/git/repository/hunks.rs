...
use super::diff::{Hunk, compare_buffer, compare_buffer_context};
use super::{private_artifact, run};
...
pub(crate) fn change_index(
    ...
    super::with_repository_transaction(workspace, |root| {
...
fn change_index_locked(
    ...
    let snapshot = super::status_at_root(root.to_owned())?;
...
mod tests {
    use super::*;
    use crate::git::repository::{args, text_run, with_repository_transaction};
    use std::fs;
    ...
    fn panel_and_hunks_from_nested_workspaces_preserve_each_others_index_changes() {
        ...
        thread::scope(|scope| {
            ...
            let panel_job = scope.spawn(|| {
                barrier.wait();
                crate::git::repository::perform(
                    &nested.join("."),
                    crate::git::repository::Operation::Stage("notes.typ".into()),
                )
            });
            barrier.wait();
    ...
    fn hunk_preconditions_are_rechecked_inside_the_repository_lease() {
        ...
        let hunk = compare_buffer(root, original.as_bytes(), &buffer)
            .unwrap()
            .remove(0);
        with_repository_transaction(root, |root| {
            // Simulate an earlier queued panel mutation winning the lease.
            fs::write(&path, "one\nnew baseline\nthree\n").unwrap();
            crate::git::repository::perform_locked(
                root,
                crate::git::repository::Operation::Stage("main.typ".into()),
            )?;
            crate::git::repository::perform_locked(
                root,
                crate::git::repository::Operation::Commit("Changed HEAD".into()),
            )?;
