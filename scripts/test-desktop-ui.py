...
class Journey:
    ...
    def snapshot(self):
        ...
        if self.viewport is not None and viewport != self.viewport:
...
def main():
    ...
    parser.add_argument("--journey", choices=["all", "panels", "find", "empty"], default="all")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="ttt-ui-", dir="/tmp") as temporary:
            ...
            fixture = directory / "journey.txt"
            fixture.write_text("Desktop journey fixture.\n")
            ...
            with (evidence / "app.log").open("w") as log:
                ...
                try:
                    ...
                    if args.observe_only:
                        ...
                        return
                    for name in (["panels", "find", "empty"] if args.journey == "all" else [args.journey]):
