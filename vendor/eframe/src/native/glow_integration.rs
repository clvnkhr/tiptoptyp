...
impl GlowWinitRunning<'_> {
    fn run_ui_and_paint(
        ...
        let (raw_input, viewport_ui_cb, is_visible, show_ui) = {
            ...
            let show_ui = crate::window_host::requires_ui(
                is_visible || is_viewport_or_descendant_visible(&glutin.viewports, viewport_id),
                viewport_info_close_requested(&glutin.viewports, viewport_id),
            );
...
impl GlutinWindowContext {
    ...
    pub(crate) fn initialize_window(
        ...
        crate::window_host::bind(&self.egui_ctx, viewport_id, window);
...
fn viewport_info_close_requested(
    viewports: &OrderedViewportIdMap<Viewport>,
    id: ViewportId,
) -> bool {
    viewports
        .get(&id)
        .is_some_and(|viewport| viewport.info.close_requested())
}
