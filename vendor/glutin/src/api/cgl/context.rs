...
impl NotCurrentGlContext for NotCurrentContext {
    ...
    fn make_current_draw_read<T: SurfaceTypeTrait>(
        ...
        Err(self
            .inner
            .make_current_draw_read(surface_draw, surface_read)
            .into())
    }
}
...
impl ContextInner {
    ...
    pub(crate) fn set_swap_interval(&self, interval: SwapInterval) {
        ...
        autoreleasepool(|_| unsafe {
            self.raw
                .setValues_forParameter(&interval, NSOpenGLCPSwapInterval);
        })
    }
    ...
    pub(crate) fn is_view_current(&self, view: &MainThreadBound<Retained<NSView>>) -> bool {
        // AppKit may enter drawRect with a different thread-current context,
        // even while our context remains attached to this view.
        is_thread_current(&self.raw)
            && run_on_main(|mtm| self.raw.view(mtm).as_ref() == Some(view.get(mtm)))
    }
...
fn is_thread_current(context: &NSOpenGLContext) -> bool {
    NSOpenGLContext::currentContext().as_deref() == Some(context)
}

#[cfg(test)]
mod current_context_tests {
    use super::super::appkit::NSOpenGLPixelFormat;
    use super::*;

    #[test]
    #[ignore = "requires macOS OpenGL; no window or event loop"]
    fn native_thread_current_context_changes_independently_of_view_attachment() {
        let format =
            unsafe { NSOpenGLPixelFormat::newWithAttributes(&[0]) }.expect("OpenGL pixel format");
        let first =
        ...
        let second =
            NSOpenGLContext::initWithFormat_shareContext(NSOpenGLContext::alloc(), &format, None)
                .unwrap();
        NSOpenGLContext::clearCurrentContext();
        assert!(!is_thread_current(&first));
        first.makeCurrentContext();
        assert!(is_thread_current(&first));
        assert!(!is_thread_current(&second));
        second.makeCurrentContext();
        assert!(!is_thread_current(&first));
        assert!(is_thread_current(&second));
        NSOpenGLContext::clearCurrentContext();
        assert!(!is_thread_current(&second));
