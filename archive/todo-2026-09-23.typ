...
170. [x] Add an embedded libghostty terminal as an alternative to the Problems panel, with one persistent workspace shell per window, keyboard input, selection/copy/paste, resize, scrollback and cleanup. Open View → Terminal or Control+Backtick; see `docs/terminal.md` for build requirements and verification.
...
181. [x] (deferred) we should be able to open from template.
...
306. [x] comfy mode (typst only) - adjust black and white (+ background color) with default set rules to match theme
307. [x] language/grammar linting for english (typst/tex aware)
308. [x] highlight invisible chars or chars commonly mistaken for other chars (e.g. cjk)
309. [x] weird chinese encoding -> utf-8 conversion (deferred until we have more details)
310. [x] full document templates
311. [x] snippet support
...
315. [x] everything currently done with text should have a swanky icon, with text going into a small tooltip instead
...
337. [x] Stage changed PDF.js documents offscreen and swap only when ready, preserving navigation and the last successful preview without a blank flash.
...
= Final backlog pass (2026-09-23)

Items 306–310, 315 and 337 were implemented in the final backlog pass.
Item 311 was stale: LSP snippets already support placeholders, defaults, choices,
variables and final cursor placement, with atomic completion-edit regressions.
Item 181 duplicates the now-implemented document template picker (310).
The completion summary and verification are in `docs/backlog-300-worklog.md`.
This archived file is historical; the root `todo.typ` is the active list.
